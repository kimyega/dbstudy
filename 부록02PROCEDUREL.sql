/*
    ...
    2. "EXCUTE 프로시저명()" 형식으로 실행할 수 있다. (프로시저 호출)
    3. 형식
        CREATE [OR REPLACE] PORCEDURE 프로시저명
        AS            -- IS를 사용해도 무방
            변수 선언 -- 생략 가능
        BEGIN
            프로시저본문
...
EXECUTE MY_PROC_02();
-- 프로시저로 값을 주는 방법
-- 사원 번호를 전달하면 해당 사원의 이름을 출력하는 프로시저
CREATE OR REPLACE PROCEDURE MY_PROC_03(EMPLOYEEID IN EMPLOYEES.EMPLOYEE_ID%TYPE)  -- 프로시저로 전달된 인수를 저장하는 입력 파라미터(변수)
IS
    FIRSTNAME EMPLOYEES.FIRST_NAME%TYPE;
    LASTNAME  EMPLOYEES.LAST_NAME%TYPE;
BEGIN
...
DECLARE 
    ...
    LASTNAME  EMPLOYEES.LAST_NAME%TYPE;
BEGIN
    MY_PROC_04(FIRSTNAME, LASTNAME);        -- 그릇에 담기 FIRSTNAME 과 LASTNAME 변수에 저장해달라. PLSQL에서 직접 프로시저를 호출할 땐 EXECUTE를 생략한다.
    DBMS_OUTPUT.PUT_LINE(FIRSTNAME || ' ' || LASTNAME);
END;
...
2. 처리할 일
    1) 제품 테이블의 재고를 수정한다.
    2) 고객 테이블의 포인트를 수정한다.
    3) 주문 테이블의 주문내역을 삽입한다.
3. 프로시저 호출 예시 
    BUY_PROC(1, 1000, 10); 고객번호 1이 제품코드 1000을 10개 구매하였다.
    
CREATE OR REPLACE PROCEDURE BUY_PROC(
    CNO IN CUST_T.C_NO%TYPE,        -- 고객번호 
    PCODE IN PROD_T.P_CODE%TYPE,    -- 제품코드
    AMOUNT IN BUY_T.B_AMOUNT%TYPE   -- 구매수량
    )
AS
BEGIN
