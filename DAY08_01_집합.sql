...
-- 부서 테이블에 존재하지만 사원테이블에 존재하지 않는 부서번호 조회하기
-- (유령부서 조회하기)
SELECT DEPARTMENT_ID
  FROM DEPARTMENTS
 MINUS
...
-- 활용 2. WITH문과 재귀 쿼리를 활용한 사원의 레벨 표시하기
-- MANAGER가 몇 명인가에 따른 LEVEL 표시하기
-- MANAGER가 0명이다 : LEVEL = 1
-- MANAGER가 1명이다 : LEVEL = 2
WITH
    MY_SUBQUERY(LVL, EMPLOYEE_ID, FIRST_NAME, LAST_NAME, MANAGER_ID) AS 
    (
    ...
    SELECT M.LVL + 1 AS LVL, E.EMPLOYEE_ID, E.FIRST_NAME, E.LAST_NAME, E.MANAGER_ID
      FROM EMPLOYEES E, MY_SUBQUERY M
