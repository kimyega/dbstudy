/*
    DDL
    1. Data Definition Langage
    2. 데이터베이스 객체를 다루는 언어
    3. 트랜잭션대상이 아니다.(작업을 취소할 수 없다.) 
    트랜잭션 : 반드시 한번에 업무 수행 EX) 은행 이체 = 출금 + 입금
    4.종류
        1) CREATE   : 생성
        2) ALTER    : 수정
        3) DROP     : 삭제
        4) TRUNCATE : 내용만 삭제 (행만)
*/
...
CREATE TABLE BANK_TBL(
    ...
    CONSTRAINT BK_TBL      PRIMARY KEY(BANK_CODE)
);
CREATE TABLE CUSTOMER_TBL(
    ...
    CONSTRAINT CUS_TBL     PRIMARY KEY(NO),
    CONSTRAINT CUS_BK_COD FOREIGN KEY(BANK_CODE) REFERENCES BANK_TBL(BANK_CODE)
);
...
--5. 고객 테이블에 GRADE 칼럼을 추가하시오. ('VIP','GOLD','SILVER'중의 하나의 값을 가지도록 한다.)
ALTER TABLE CUSTOMER_TBL ADD GRADE VARCHAR(6 BYTE) NOT NULL CHECK(GRADE = 'VIP' OR GRADE = 'GOLD' OR GRADE = 'SILVER' OR GRADE = 'BRONZE');
ALTER TABLE CUSTOMER_TBL ADD GRADE VARCHAR(6 BYTE) NOT NULL CHECK(GRADE IN('VIP','GOLD','SILVER','BRONZE'));
--6. 고객 테이블의 고객명(NAME)과 연락처(PHONE) 칼럼이름을 CUST_NAME, CUST_PHONE으로 변경하시오.
...
ALTER TABLE CUSTOMER_TBL MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL; --반드시 NULL을 명시해야 한다.ALTER에서 안적으면 바뀌지 않는다.
--테이블 구조 확인하기
DESCRIBE BANK_TBL; --DESC까지 지우기 가능
DESC CUSTOMER_TBL;
