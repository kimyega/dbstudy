...
/*
    ...
    1. NUMBER(P,S) : 정밀도 p, 스케일 s로 표현하는 숫자 형식
        1) 정밀도 p : 전체유효숫자(문자 X)의 갯수   ex) 0.99 앞의 0은 유효 x
        2) 스케일 s : 소수부의 유효 숫자의 갯수
    2. CHAR(size) : 고정 문자(character) 
        1) 글자수가 고정된 타입 핸드폰 ex) 핸드폰 번호, 주민번호 등
        2) size : 최대 2000 byte
    3. VARCHAR2(size) : 가변 문자
        1) 글자 수가 고정되지 않은 타입 ex) 이름 이메일 주소 등
        2) size : 최대 4000 byte
    4. CLOB : 큰 텍스트 타입 
    5. DATE : 날짜와 시간을 동시에 표현하는 타입 ex) 년, 월, 일 ,시, 분, 초 
    6. TIMESTAMP : 날짜와 시간을 동시에 표현하는 ex) 타입 년, 월, 시, 분, 초, 마이크로초
*/
...
CREATE TABLE CUSTOMER_T (
    ...
    JUBUN   CHAR(13 BYTE)     NOT NULL   UNIQUE
);
