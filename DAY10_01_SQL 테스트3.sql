...
SELECT PROF_NAME AS 교수명,
       TRUNC(TO_DATE('2020-12-31','YYYY-MM-DD')) - TRUNC(TO_DATE(PROF_HIRED)) AS 근속일
  FROM PROFESSOR_T;
...
SELECT STUD_NO      AS 학번,
       ...
       STUD_HEIGHT  AS 키
  FROM STUDENT_T
 WHERE STUD_HEIGHT IN ((SELECT MAX(STUD_HEIGHT)
                          FROM STUDENT_T), 
                       (SELECT MIN(STUD_HEIGHT)
                          FROM STUDENT_T));
